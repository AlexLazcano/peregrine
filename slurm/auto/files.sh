    ...
    for motif_type in "${pattern[@]}"; do
        for n_tasks in "${n_tasks_values[@]}"; do
            for cpu_count in "${total_cpus[@]}"; do
                ...
                script_content="$(generate_job_command "$data_dir" "$motif_type" $cpus_per_task $time_limit $mem_limit $n_tasks)\n\n"
                # Write the script content to the output script file
                output_script="./$directory/$motif_type.$n_tasks.$cpus_per_task.sh"
